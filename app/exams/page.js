  ...
  return (
    <main className="min-h-screen bg-slate-900 text-slate-50 font-sans">
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        ...
        <div className="bg-slate-800/50 border-l-4 border-blue-500 p-4 rounded-r-lg mb-8">
          <p className="text-slate-300 text-sm font-semibold flex items-center gap-2 mb-1">
            <span>ℹ️</span> {e.noticeTitle}
          </p>
          <p className="text-slate-400 text-xs">{e.noticeDesc}</p>
        </div>
            ...
            <div key={i} className={`bg-slate-800 border border-slate-700/60 border-l-4 ${exam.color} rounded-xl p-6 hover:shadow-[0_0_15px_rgba(59,130,246,0.05)] hover:scale-[1.01] transition-all duration-200 ease-in-out`}>
              <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                <div className="flex-1">
                  ...
                  <p className="text-slate-400 text-sm mb-2">{exam.fullName}</p>
                  <p className="text-slate-200 text-sm">{exam.desc[lang]}</p>

                  <div className="flex flex-wrap gap-x-6 gap-y-2 mt-4">
                    <div>
                      <p className="text-slate-500 text-xs">{e.examDate}</p>
                      <p className="text-white text-sm font-semibold">{exam.date}</p>
                    </div>
                    <div>
                      <p className="text-slate-500 text-xs">{e.applyBefore}</p>
                      <p className="text-white text-sm font-semibold">{exam.applyDate}</p>
                    </div>
                    <div>
                      <p className="text-slate-500 text-xs">{e.eligibility}</p>
                      <p className="text-white text-sm font-semibold">{exam.eligibility}</p>
                    </div>
                    <div>
                      <p className="text-slate-500 text-xs">{e.forStreams}</p>
